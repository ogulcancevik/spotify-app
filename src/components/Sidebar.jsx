...
const Sidebar = () => {
  ...
  return (
    <div className="w-64 flex flex-col items-center text-gray-300 rounded-xl ml-1" style={{
      background: '#121212',
      height: 'calc(100vh - 125px)'
    }}>
      <div className="flex items-center w-full px-4 mt-6">
        <ul className="flex flex-col justify-around h-24">
          <li className="flex items-center cursor-pointer hover:text-white" onClick={() => {
            navigate('/')
          }} >
      ...
      <div className="px-4 overflow-auto" style={{
        ...
        {userPlayList
          ? userPlayList.map(({ id, name }) => {
            ...
            }} className="my-6 hover:text-white cursor-pointer" key={id}>
              {name}
            </div>
            )
          })
